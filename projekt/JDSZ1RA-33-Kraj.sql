...
konwersja_agr AS
  (select id_skutecznej_kampanii, kraj, count(id_wniosku) convert
      from konwersja
        group by 1,2),
...
join m_dane_od_partnerow mdop ON l.id = mdop.id),
  ...
  mianownik_agr AS
  (select id_kampanii, kraj, count(id_lead) sent
      from mianownik
        group by 1,2),

summary as
  (select m.id_kampanii, m.kraj, coalesce(k.convert, 0) convert, coalesce(m.sent, 0) sent
from mianownik_agr m
left join konwersja_agr k on (m.id_kampanii=k.id_skutecznej_kampanii) and m.kraj = k.kraj),
...
WoE as (select kraj as jWoE, ln(DG/DB) WoE_val from base),

DG_DB_diff as (select kraj as jDiff, (DG - DB) DDiff from base),

IV_Part as (select jWoe as kraj, DDiff * WoE_val as IV from DG_DB_diff join WoE on jDiff = jWoE)

  --select sum(IV) from IV_Part

select b.*, w.WoE_val, d.DDiff, i.IV from base b
join WoE w on b.kraj = w.jWoE
join DG_DB_diff d on b.kraj = d.jDiff
join IV_Part i on b.kraj = i.kraj
